...
export class TodoService {
  apiURL = 'http://localhost:3000'; //'https://todolist.zoltanhalasz.net/api';
  ...
  getTodoList(): Observable<todomodel[]> {
    return this.http
      .get<todomodel[]>(this.apiURL + "/ToDoModels")
  ...
  getTodo(id): Observable<todomodel> {
    return this.http
      .get<todomodel>(this.apiURL + "/ToDoModels/" + id)
      .pipe(retry(1), catchError(this.handleError));
  }
  ...
  deleteTodo(id) {
    return this.http
      .delete<todomodel>(this.apiURL + "/ToDoModels/" + id, this.httpOptions)
      .pipe(retry(1), catchError(this.handleError));
  }
